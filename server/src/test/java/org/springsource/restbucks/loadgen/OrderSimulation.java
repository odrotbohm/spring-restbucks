                ...
                .on(pause(1)
                    .exec(
                      ...
                      try {
                        ...
                        JsonNode orderStatusLinkNode = rootNode.at("/status");
                      ...
                      return session.set("receipt", receipt != null ? receipt.getHref() : null)
                          .set("orderStatus", orderStatus);
                    })
              ).exec(
                http("Take Order")
                    .delete(session -> session.get("receipt"))
                    .check(status().in(200, 201, 404, 500))// Expecting valid status
              )
      );
